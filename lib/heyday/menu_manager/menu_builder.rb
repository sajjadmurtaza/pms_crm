  ...
  module MenuManager
    class MenuBuilder
      def initialize(menu, items)
        items[menu] ||= Heyday::MenuManager::TreeNode.new(:root, {})
        @menu = menu
      ...
      #   eg. :children => Proc.new {|project| [Heyday::MenuManager::MenuItem.new(...)] }
      ...
      def push(name, url, options={})
        ...
        if first = options.delete(:first)
          target_root.prepend(Heyday::MenuManager::MenuItem.new(name, url, options))
        elsif before = options.delete(:before)

          if exists?(before)
            target_root.add_at(Heyday::MenuManager::MenuItem.new(name, url, options), position_of(before))
          else
            target_root.add(Heyday::MenuManager::MenuItem.new(name, url, options))
          end

        elsif after = options.delete(:after)

          if exists?(after)
            target_root.add_at(Heyday::MenuManager::MenuItem.new(name, url, options), position_of(after) + 1)
          else
            target_root.add(Heyday::MenuManager::MenuItem.new(name, url, options))
          end

        elsif options[:last] # don't delete, needs to be stored
          target_root.add_last(Heyday::MenuManager::MenuItem.new(name, url, options))
        else
          target_root.add(Heyday::MenuManager::MenuItem.new(name, url, options))
        end
      end
